fun day13_1(lines: List<String>): Int {
    var mirrorSum = 0

    val list = mutableListOf<String>()
    (lines + "").forEach { line ->
        if (line.isNotEmpty()) {
            list.add(line)
        } else {
            mirrorSum += list.hor() ?: list.vert() ?: throw IllegalStateException()
            list.clear()
        }
    }
    return mirrorSum
}

fun List<String>.hor(): Int? = this.map { it.toBinNum() }.mirrorIndex()?.let { it * 100 }
fun List<String>.vert(): Int? = this.pivot().map { it.toBinNum() }.mirrorIndex()
fun String.toBinNum(): Int = this.replace('.', '0').replace('#', '1').toInt(2)
fun List<String>.pivot(): List<String> =
    (0..this[0].lastIndex).map { idx -> this.map { it[idx] } }.map { it.joinToString("") }

fun List<Int>.mirrorIndex(): Int? {
    if (this.size <= 1) return null
    val mid = this.size / 2
    (1..mid).reversed().forEach { idx ->
        if (this.mirroredOnIndex(idx)) {
            return idx
        }
        if (this.reversed().mirroredOnIndex(idx)) {
            return mid + (mid - idx) + 1
        }
    }
    return null
}

fun List<Int>.mirroredOnIndex(index: Int): Boolean {
    val left = this.subList(0, index)
    val rest = this.subList(index, index * 2)
    return left == rest.reversed()
}

